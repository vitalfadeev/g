...
BOOL SetKeyAndValue(LPCSTR pszKey, LPCSTR pszSubkey, LPCSTR pszValue)
{
	...
	if (null != pszValue)
	{
	    if (RegSetValueExA(hKey, null, 0, REG_SZ, cast(BYTE *) pszValue,
                           to!uint( (strlen(pszValue) + 1) * char.sizeof ) ) != ERROR_SUCCESS)
		result = false;
	}
...
int dll_regserver(const (char) *dllname, int flag) {
	LPSTR fn = flag ? cast(char*) "DllRegisterServer" : cast(char*) "DllUnregisterServer";
	int result = 1;
	pfn_t pfn;
	HINSTANCE hMod;
